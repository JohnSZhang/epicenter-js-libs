...
"use strict";function setRunInSession(sessionKey,run){var path="/"+[urlService.appPath,urlService.accountPath,urlService.projectPath].join("/");path=path.replace(/\/{2,}/g,"/"),sessionStore.set(sessionKey,JSON.stringify({runId:run.id}),{root:path})}var makeSeq=require("../../util/make-sequence");var Base=require("./identity-strategy");var SessionStore=require("../../store/store-factory");var classFrom=require("../../util/inherit");var UrlService=require("../../service/url-config-service");var AuthManager=require("../auth-manager");var sessionStore=new SessionStore({});var urlService=new UrlService;var keyNames=require("../key-names");var defaults={sessionKey:keyNames.STRATEGY_SESSION_KEY};var Strategy=classFrom(Base,{constructor:function(runService,condition,options){if(null==condition)throw new Error("Conditional strategy needs a condition to createte a run");this._auth=new AuthManager,this.run=makeSeq(runService),this.condition="function"!=typeof condition?function(){return condition}:condition,this.options=$.extend(!0,{},defaults,options),this.runOptions=this.options.run},reset:function(runServiceOptions){var _this=this;var session=this._auth.getCurrentUserSessionInfo();var opt=$.extend({scope:{group:session.groupId}},this.runOptions);return this.run.create(opt,runServiceOptions).then(function(run){return setRunInSession(_this.options.sessionKey,run),run.freshlyCreated=!0,run}).start()},getRun:function(){var session=this._auth.getCurrentUserSessionInfo();return session&&session.runId?this._loadAndCheck(session):this.reset()},_loadAndCheck:function(session){var shouldCreate=!1;var _this=this;return this.run.load(session.runId,null,{success:function(run,msg,headers){shouldCreate=_this.condition.call(_this,run,headers)}}).then(function(run){return shouldCreate?_this.run.original.create(_this.runOptions).then(function(run){return setRunInSession(_this.options.sessionKey,run),run.freshlyCreated=!0,run}):run}).start()}});module.exports=Strategy;
...
module.exports={"new-if-initialized":require("./new-if-initialized-strategy"),"new-if-persisted":require("./new-if-persisted-strategy"),"new-if-missing":require("./new-if-missing-strategy"),"always-new":require("./always-new-strategy"),multiplayer:require("./multiplayer-strategy"),"persistent-single-player":require("./persistent-single-player-strategy"),none:require("./identity-strategy"),"base-if-initialized":require("./base-if-initialized-strategy")};
